...
Learn More: https://nextjs.org/docs/messages/node-module-in-edge-runtime`;
  ...
  `);
      ...
      let re = ["/", "/checkout", "/not-found", "/termos-de-uso", "/politica-devolucao", "/produto"], rt = ["/config-page", "/dashboard", "/cashback"];
      async function rr(e10) {
        try {
          let t10 = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/api/auth/me`, { headers: { Cookie: `mupi_session=${e10}` } });
          if (!t10.ok) return null;
          let r2 = await t10.json();
          return r2.user ? { role: r2.user.role } : null;
        } catch (e11) {
      ...
      async function rn(e10) {
        ...
        if (re.some((e11) => t10 === e11 || t10.startsWith(e11 + "/")) || t10.startsWith("/api/")) return es.next();
        let r2 = e10.cookies.get("mupi_session")?.value;
        if (!r2) return es.redirect(new URL("/", e10.url));
        let n2 = await rr(r2);
        return n2 ? rt.some((e11) => t10 === e11 || t10.startsWith(e11 + "/")) && "master" !== n2.role ? es.redirect(new URL("/", e10.url)) : es.next() : es.redirect(new URL("/", e10.url));
      }
      e.s(["config", 0, { matcher: ["/((?!_next|static|favicon.ico|api).*)"] }, "middleware", 0, rn], 99446);
      let ri = { ...e.i(99446) }, ra = "/middleware", ro = ri.middleware || ri.default;
      ...
      let rs = (e10) => t_({ ...e10, IncrementalCache: t8, incrementalCacheHandler: null, page: ra, handler: async (...e11) => {
        try {
          return await ro(...e11);
        } catch (i2) {
